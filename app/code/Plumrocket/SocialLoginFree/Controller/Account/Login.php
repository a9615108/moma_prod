...
class Login extends \Plumrocket\SocialLoginFree\Controller\AbstractAccount
{
    ...
    public function execute()
    {
        ...
        if (!$addresss->getData()) {
            $redirectUrl = $this->_objectManager->get('Magento\Store\Model\Store')->getBaseUrl() . 'customer/address';
        }

        $this->Customer  = $this->_objectManager->create('Magento\Customer\Model\Customer');
        $customer_eav = $this->Customer->load( $customerId );
        if( $customer_eav->getVipNum() == ''){
            // 沒有 VIP 卡號 驗證電話號碼

            $redirectUrl = '/customer_account/Loginpost/Loginverification';
        }
        ...
        if ($this->getRequest()->isXmlHttpRequest()) {
            ...
            ';
            // var_dump($jsAction);die();
            $body = $this->_jsWrap('if(window.opener && window.opener.location &&  !window.opener.closed) {  }; '.$jsAction.';');
